...
def avenb(nbfix,cut):
	...
	for nbf in nbfix:
		...
		for nb in nbf:
			...
			i = int(ii[1:])
			j = int(jj[1:])
			if i not in nbtmp.keys() and j not in nbtmp.keys():
				nbtmp[i] = {}
				nbtmp[i][j] = [ eij, rij, 1 ]
			elif i in nbtmp.keys():
				if j in nbtmp[i].keys():
					nbtmp[i][j][0] += eij
					nbtmp[i][j][1] += rij
					nbtmp[i][j][2] += 1
				else:
					nbtmp[i][j] = [ eij, rij, 1 ]
			elif j in nbtmp.keys():
				if i in nbtmp[j].keys():
					nbtmp[j][i][0] += eij
					nbtmp[j][i][1] += rij
					nbtmp[j][i][2] += 1
				else:
	...
	for i in nbt_keys:
		...
		for j in ikeys:
			...
			if float(freq)/float(nstr) > cut:
				eij = nbtmp[i][j][0]/float(freq)
				rij = nbtmp[i][j][1]/float(freq)
				avenb.append(("G%i"%i,"G%i"%j,eij,rij))
...
nbatt,avennb = avenb(map(lambda x: x[7][1:], parmdat),cfrac)
