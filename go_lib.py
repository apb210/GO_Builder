...
import sys,math,getopt,os,random

# globals ==========================================================================

HOME = os.getenv('HOME')
DATA_DIR = "%s/ResearchGroup/Go_Builder/March19_2011"%HOME
#crowdmass = 1000.0
...
ecrowd_std = -1.0
# big water box:
#std_boxlen = 40.33	# Angstroms
#std_nwat = 560
#std_wbox = "%s/bigbox.pdb" % DATA_DIR
# little water box:
...
std_sigrep = 3.0
#std_sc_sigrep = 4.0	# this value seems a bit large!
std_bb_sigrep = 6.0
std_sc_sigrep = 3.0
hcut_theta = 135.0
hcut_dist = 3.4
...
massmap = { 'ALA': 71.0, 'GLY': 57.0,'THR': 101.0,'TYR': 163.0,
	...
	'CRO': 500.0, 'A': 328., 'T': 319., 'C': 304., 'G':344., 'U':305. }
...
## "sc_water3"
#wmap_sc = { 'CA': 0.6, 'TYR': 0.2,
#	'VAL': 0.2, 'LEU': 0.2,'ILE': 0.2,'TRP': 0.2,
#	'GLU': 1.2, 'ASP': 1.2, 'ASN': 0.81,
#	'GLN': 0.81, 'PHE': 0.2,'ARG': 1.2,
#	'HIS': 1.2,'LYS': 1.2,'MET': 0.2 }

# "sc_water4"
#wmap_sc = { 'CA': 0.7, 'TYR': 0.3,
#	'VAL': 0.3, 'LEU': 0.3,'ILE': 0.3,'TRP': 0.3,
#	'GLU': 1.2, 'ASP': 1.2, 'ASN': 0.81,
...
scbondmap = { 'TYR': 3.7, 'LEU': 2.6, 'ILE':2.3, 'TRP': 3.9,
		...
		'MET': 2.7, 'VAL': 2.0, 'A': 4.5, 'T': 4.0,
		'C': 4.0, 'G': 4.5, 'U': 4.0 }

##              angle      k1     th1   k2    th2   beta  epsilon1
#statangle = { 'BACKBONE': [106.4, 91.7, 26.3, 130.0, 0.1,  4.3],
#	      'TYR1':  [9.032, 95.1,  9.032, 162.5, 1.0, -0.405],
#	      'TYR2': [ 7.415, 88.1,  7.415, 152.8, 1.0, 0.118],
#	      'VAL1': [20.947, 117.4],
#	      'VAL2': [20.947, 117.4],
#	      'LEU1': [19.736, 110.5, 19.736, 144.7, 1.0, -0.667],
#	      'LEU2': [12.235, 103.9, 12.235, 137.1, 1.0, 0.510],
#	      'ILE1': [14.849, 120.1],
#	      'ILE2': [14.849, 120.1],
#	      'TRP1': [ 5.672, 101.5,  5.672, 156.5, 1.0, -0.363],
#	      'TRP2': [ 3.036,  85.1,  3.036, 141.7, 1.0, -0.002],
#	      'GLU1': [ 5.753, 121.5],
#	      'GLU2': [ 5.753, 121.5],
#	      'ASP1': [14.089, 106.1, 14.089, 146.5, 1.0, -0.562],
#	      'ASP2': [10.822, 100.1, 10.822, 142.6, 1.0, 0.144],
#	      'SER1': [ 6.820, 113.3],
#	      'SER2': [ 6.820, 113.3],
#	      'ASN1': [13.465, 104.5, 13.465, 145.7, 1.0, -0.591],
#	      'ASN2': [11.020, 100.3, 11.020, 143.2, 1.0, 0.190],
#	      'GLN1': [ 5.146, 122.2],
#	      'GLN2': [ 5.146, 122.2],
#	      'PHE1': [10.822, 97.1, 10.822, 158.7, 1.0, -0.564],
#	      'PHE2': [ 8.707, 91.6,  8.707, 151.0, 1.0, 0.198],
#	      'ARG1': [ 3.798, 121.0],
#	      'ARG2': [ 3.798, 121.0],
#	      'HIS1': [11.259, 96.4, 11.259, 153.3, 1.0, -0.657],
#	      'HIS2': [ 8.935, 93.6,  8.935, 151.1, 1.0, 0.341],
#	      'LYS1': [ 4.641, 124.4],
#	      'LYS2': [ 4.641, 124.4],
#	      'MET1': [ 5.445, 121.1],
#	      'MET2': [ 5.445, 121.1] }
...
#statangle = { 'BACKBONE': [106.4, 87.7, 26.3, 130.0, 0.08,  5.0],
statangle = { 'BACKBONE': [106.4, 91.7, 26.3, 130.0, 0.1,  4.3], ### OLD
...
def isNA(crds):
	if crds[1]['name'].strip() in [ 'A','C', 'T','G','U' ]:
		return 1
	else:
		return 0
...
def make_coords(pdblines,charmm=1):
	...
	for line in pdblines:
		...
		if chain != pchain:
			isNA = 0
			if res_name.strip() in [ 'A', 'T', 'C', 'G', 'U' ]:
				isNA = 1
			pchain = chain
			pres = 0
			residue = 0
		...
		#print atom
		ast = atom.strip()
		# heme is special case
		#if res_name == "HEM" or res_name == "HEME":
		#	if as in [ "FE","C2A","C3A","C2B","C3B","C2C","C3C","C2D","C3D" ]:
		#		crd_dat[chain][residue][as] = \
		#				(float(line[30:38]),float(line[38:46]),float(line[46:54]))
		#	elif as[0] != "H":
		#		if (len(as)>1):
		#			if ( as[1]=="H" and as[0].isdigit() ):
		#				continue
		#
...
#def calc_ncon_intra(coords,Tf,ngdist,gamma,geom=0,fixrep=0,hb_ecut=-0.5):
...
def calc_ncon_intra_na(coords,ngdist,gamma,geom=0,fixrep=0,hb_ecut=-0.5):
	"""The most important function: create go/non-go lists for inclusion
	in parameter file or golist file
	This version calculates contacts within a single chain
	...
	## method 2: thirumalai: bb contacts < 14 A
	#for i in range(1,nres-2):
	#	for j in range(i+3,nres+1):
	#		rij = dist(coords[i]["C1'"],coords[j]["C1'"])
	#		if rij < dt_cut:
...
#def calc_ncon_inter(crda,crdb,Tf,ngdist,gamma,geom=0,fixrep=0,hb_ecut=-0.5):
...
#def merge_sig_matrix2(nbmat,xyz):
#	chains = xyz.keys()
#	chains.sort()
#	nchain = len(chains)
#	offsets = {}
...
#			chainj = chains[j]
#			offj = offsets[chainj]
#			for k in nbmat[chaini][chainj].keys():
#				msig[k+offi,k+offj] = maps[chaini][chainj][k]
#			for k in nbmat[
#			if ci == cj:
#				matsig = nbmat[ci][cj]['sig']
#			else:
...
def setup_dihe(crds,link,fixalpha=0,zero=0):
	dihelist = []
	nres = len(crds.keys())
...
def make_CA_linear(bonds, angles):
	"""write CA-only coordinates, possibly with multiple chains"""
...
def write_gro_top(filename,key,residues, bonds, angles, dihedrals,
		sig_rep,nbfix,nsfac,tfbb,seqs,symm,winte,potential,khmap,addcharges,intrakh):
	...
	for i in range(nres):
		...
		#		% (res,res,mass,0.0,sigma,eij_rep))
	...
	#	for chain in seqs
	#print residues
	#print seqs[seqs.keys()[0]]
	#if 'G24' in seqs[seqs.keys()[0]]:
	#	print "YES"
	#else:
	#	print "NO"
	...
	for chain in chains:
		...
		for dihe in dihedrals[chain]:
			r1,r2,r3,r4 = dihe[0],dihe[1],dihe[2],dihe[3]
			for p in dihe[4]:
				k = p[0]*gmx_dihe_fac
				mult = p[1]
				phi0 = p[2]
