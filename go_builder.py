...
import sys,os,go_lib,getopt,copy
...
Usage:
	...
	--fixalpha --uniform file.pdb --ignore-inter --bind-n-go
	...
	in which:
		...
		--uniform : make all contacts the same energy
...
optlist, files = getopt.getopt(sys.argv[1:], 'hqpc:t:k:n:g:d:l:r:w:sS:B:GAD:Os:H3:', \
		...
		'ncrowd=','rcrowd=','crowdbox=','crowdprot=','uniform','fixalpha',
		'ignore-inter','bind-n-go'])
...
for i in range(nchain):
	...
	for j in range(i+1,nchain):
		...
		if ignore_inter:
			...
			nonbondedx[chaini][chainj] = go_lib.calc_ncon_inter(meta_crd[chaini],meta_crd[chainj],\
...
if (gmx):
	...
	go_lib.write_gro_top(outp_base+"_gromacs_go.top",key,residues,unmerged_bonds,
			...
			go_and_nongo,NSFAC,tfbb,seqs,symm,winte,"12_10_6",{},0,1)
	if nchain > 1:
